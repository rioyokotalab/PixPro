...
def debug_img_save(img_tensor, orig_img_pil, out_coord, out_grid, idx=1, calc_coord=None, is_corner=True):
    ...
    img_str = add_str_img(f"official data aug img on {idx} frame", img)
    ...
    # img_tmp_resize = add_str_img(f"down size my resized data aug img on {idx} frame by pad", img_tmp_resize)
    ...
    if is_use_calc_coord:
        ...
        img_calc_tmp = add_str_img(f"data aug by using official grid img on {idx} frame", img_calc_tmp[0])
        inv_img_calc_tmp = nnF.grid_sample(img.unsqueeze(0), calc_grid_tmp, align_corners=is_corner)
        inv_img_calc_tmp = to_pad_resize(inv_img_calc_tmp)
        inv_img_calc_tmp = add_str_img(f"inv data aug by using official grid img from officical data aug on {idx} frame", inv_img_calc_tmp[0])

    down_size_orig_img = to_pad_resize(orig_img[0])
    # down_size_orig_img = to_pad_resize(orig_img)
    down_size_orig_img = add_str_img(f"down size orginal img on {idx} frame", down_size_orig_img)

    down_size_img_tmp = to_pad_resize(img_tmp[0])
    # down_size_img_tmp = to_pad_resize(img_tmp)
    # down_size_img_tmp = down_size_img_tmp[0]
    down_size_img_tmp = add_str_img(f"down size my data aug img on {idx} frame by pad", down_size_img_tmp)
    ...
    img_tmp_crop = add_str_img(f"down size my data aug img on {idx} frame by resize", img_tmp_crop)

    # inv_img = nnF.grid_sample(img.unsqueeze(0), coord_tmp, align_corners=is_corner)
    # inv_img = add_str_img(f"inv my data aug img from offical data aug img on {idx} frame", inv_img[0])
    # inv_img_tmp = nnF.grid_sample(img_tmp, coord_tmp, align_corners=is_corner)
    # inv_img_tmp = add_str_img(f"inv my data aug img from my data aug img which no resize on {idx} frame", inv_img_tmp[0])
    # inv_img_tmp_crop = nnF.grid_sample(img_tmp_crop.unsqueeze(0), coord_tmp, align_corners=is_corner)
    # inv_img_tmp_crop = add_str_img(f"inv my data aug img from my data aug img which has resize on {idx} frame", inv_img_tmp_crop[0])
