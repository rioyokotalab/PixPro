...
class Resize:
    ...
    def __call__(self, sample):
        ...
        image = image.resize((self.new_width, self.new_height), Image.BICUBIC)
        label = label.resize((self.new_width, self.new_height), Image.NEAREST)
...
class RandomResize:
    ...
    def __init__(self, min_height=512, max_height=2048, ratio=1.0):
    ...
    def __call__(self, sample):
        ...
        height_now = random.randint(self.min_height, self.max_height)
        width_now = int(ow * height_now * self.ratio / oh)
        image = image.resize((width_now, height_now), Image.BICUBIC)
        label = label.resize((width_now, height_now), Image.NEAREST)
